...
def plot_color_trends(df, top_n=10):
    ...
    fig = go.Figure(data=[go.Bar(
        x=top_colors.index, y=top_colors.values,
        marker_color=config.COLOR_PALETTE,
        text=top_colors.values, textposition='auto'
    )])
    fig.update_layout(title="Top Trending Colors", xaxis_title="Color", yaxis_title="Count", height=config.CHART_HEIGHT)
    return fig
...
def plot_style_trends(df, top_n=10):
    ...
    fig = go.Figure(data=[go.Bar(
        x=top_styles.index, y=top_styles.values, marker_color=config.COLOR_PALETTE,
        text=top_styles.values, textposition='auto'
    )])
    fig.update_layout(title="Top Trending Styles", xaxis_title="Style Attribute", yaxis_title="Count", height=config.CHART_HEIGHT)
    return fig
...
def plot_brand_trends(df, top_n=10):
    ...
    fig = go.Figure(data=[go.Bar(
        x=top_brands.index, y=top_brands.values, marker_color=config.COLOR_PALETTE,
        text=top_brands.values, textposition='auto'
    )])
    fig.update_layout(title="Top Trending Brands", xaxis_title="Brand", yaxis_title="Count", height=config.CHART_HEIGHT)
    return fig
