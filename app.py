...
def page_overview(df, metrics):
    ...
    # Use original numeric columns for avg
    def safe_mean(df_subset, cols):
        return {col: df_subset[f"{col}_orig"].mean() if f"{col}_orig" in df_subset.columns else None for col in cols}

    numeric_cols = ['price','rating']
    trending_avg = safe_mean(trending, numeric_cols)
    non_trending_avg = safe_mean(non_trending, numeric_cols)
...
def page_trend_insights(df, feature_importance):
    ...
    numeric_cols = ['price', 'rating']
    trending_avg = {col: trending[f"{col}_orig"].mean() if f"{col}_orig" in trending.columns else trending[col].mean() for col in numeric_cols}
    non_trending_avg = {col: non_trending[f"{col}_orig"].mean() if f"{col}_orig" in non_trending.columns else non_trending[col].mean() for col in numeric_cols}
