...
def page_eda(raw_df, df):
    ...
    # Top categories & brands (raw)
    cols = st.columns(2)
    if 'category' in raw_df.columns:
        cols[0].bar_chart(raw_df['category'].value_counts().head(config.TOP_N))
    if 'brand' in raw_df.columns:
        cols[1].bar_chart(raw_df['brand'].value_counts().head(config.TOP_N))
