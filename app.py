...
def page_eda(raw_df, df):
    ...
    if 'rating' in raw_df.columns:
        st.markdown("Distribution of Ratings (raw)")
        fig, ax = plt.subplots()
    ...
    st.write("### Top Categories & Brands (filtered)")
    c1, c2 = st.columns(2)
    ...
    if 'category' in df_filtered.columns and metric in df_filtered.columns:
        top_categories_count = df_filtered['category'].value_counts().head(config.TOP_N)
        top_categories_metric = df_filtered.groupby('category')[metric].mean().loc[top_categories_count.index]
        top_categories_df = pd.DataFrame({
            "Count": top_categories_count,
            f"Avg {metric}": top_categories_metric
        })
        c1.write("**Top Categories Table**")
        c1.dataframe(top_categories_df, use_container_width=True)
    ...
    # Word clouds from raw text columns
    st.markdown("Word Clouds (from raw text columns)")
