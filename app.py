...
def page_trend_insights(df, feature_importance):
    ...
    with cols_left:
        ...
        st.write({
            "avg_price": float(trending['price'].mean()) if 'price' in trending.columns else None,
            "avg_rating": float(trending['rating'].mean()) if 'rating' in trending.columns else None
        })
    with cols_right:
        non = df[df['Trend'] == 0]
        st.write("Non-trending items averages")
        st.write({
            "avg_price": float(non['price'].mean()) if 'price' in non.columns else None,
            "avg_rating": float(non['rating'].mean()) if 'rating' in non.columns else None
        })
