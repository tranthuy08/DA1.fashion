...
def train_and_get_model(df):
    """Train model on df (df already has scaled numeric columns)."""
...
def page_eda(raw_df, df):
    st.markdown('<div class="sub-header">📈 EDA Report (Interactive)</div>', unsafe_allow_html=True)
    sns.set(style="whitegrid")
    
    st.write("### Dataset Shape & Missing Values")
    st.write("Shape (raw_df):", raw_df.shape)
    ...
    trend_options = df['Trend'].unique().tolist() if 'Trend' in df.columns else [0,1]
    trend_filter = st.sidebar.multiselect("Filter by Trend", options=trend_options, default=trend_options)
    
    if 'category' in df.columns:
        categories = df['category'].unique().tolist()
        category_filter = st.sidebar.multiselect("Filter by Category", options=categories, default=categories)
    else:
        category_filter = df['category'] if 'category' in df.columns else []

    if 'brand' in df.columns:
        brands = df['brand'].unique().tolist()
        brand_filter = st.sidebar.multiselect("Filter by Brand", options=brands, default=brands)
    else:
        brand_filter = df['brand'] if 'brand' in df.columns else []

    if 'season' in df.columns:
        seasons = df['season'].unique().tolist()
        season_filter = st.sidebar.multiselect("Filter by Season", options=seasons, default=seasons)
    else:
        season_filter = df['season'] if 'season' in df.columns else []
    ...
    if 'rating' in df_filtered.columns and not df_filtered.empty:
        ...
        sns.histplot(df_filtered['rating'].dropna(), bins=30, kde=True)
        ax.set_xlabel("Rating")
        ax.set_ylabel("Frequency")
        st.pyplot(fig)
        plt.close(fig)

        st.write("### Rating Distribution by Trend")
        fig, ax = plt.subplots(figsize=figsize)
        sns.boxplot(x='Trend', y='rating', data=df_filtered, palette="Set2", ax=ax)
    ...
    if 'category' in df_filtered.columns:
        ...
        c1.bar_chart(top_categories)
    if 'brand' in df_filtered.columns:
        top_brands = df_filtered['brand'].value_counts().head(config.TOP_N)
        c2.bar_chart(top_brands)

    # Trend distribution by season
    if 'season' in df_filtered.columns and 'Trend' in df_filtered.columns:
        st.write("### Trend Distribution by Season")
        fig, ax = plt.subplots(figsize=figsize)
        sns.countplot(x='season', hue='Trend', data=df_filtered, palette="Set1", ax=ax)
    ...
    # Correlation matrix
    numeric_cols = [c for c in ['trend_score', 'Trend'] if c in df_filtered.columns]
    ...
    text_cols = {
        ...
        "fashion influencers": "Fashion Influencers",
        "social media comments": "Social Media Comments"
    }
    for col, title in text_cols.items():
        if col in df_filtered.columns:
            text = " ".join(df_filtered[col].dropna().astype(str))
            if text.strip():
    ...
    if 'Trend' in df_filtered.columns:
        st.write("### Distribution of Product Trend Labels")
        fig, ax = plt.subplots(figsize=figsize)
        sns.countplot(x='Trend', data=df_filtered, palette="Set2", ax=ax)
        ax.set_xlabel("Trend (0 = Not Trending, 1 = Trending)")
        ax.set_ylabel("Count")
        st.pyplot(fig)
        plt.close(fig)
