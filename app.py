...
def page_overview(df, metrics):
    ...
    try:
        trending_ratio_str = f"{float(trending_ratio)*100:.2f}%"
    except (TypeError, ValueError):
        trending_ratio_str = "N/A"

    try:
        avg_price_str = f"${float(avg_price):.2f}" if avg_price is not None else "N/A"
    except (TypeError, ValueError):
        avg_price_str = "N/A"

    try:
        avg_rating_str = f"{float(avg_rating):.2f}" if avg_rating is not None else "N/A"
    except (TypeError, ValueError):
        avg_rating_str = "N/A"
...
def page_trend_insights(df, feature_importance):
    ...
    trending = df[df['Trend']==1]
    if trending.empty: st.info("No trending items detected."); return

    c1, c2, c3 = st.columns(3)
    with c1: st.write(trending['color'].value_counts().head(10))
    with c2: st.write(trending['style attributes'].value_counts().head(10))
    with c3: st.write(trending['brand'].value_counts().head(10))

    st.markdown("---")
    st.markdown("#### Statistical Summary (Trending vs Non-Trending)")
    trending_avg = trending[['price_orig','rating_orig']].mean(numeric_only=True).to_dict()
    non_trending_avg = df[df['Trend']==0][['price_orig','rating_orig']].mean(numeric_only=True).to_dict()
    cols_left, cols_right = st.columns(2)
    cols_left.write("Trending items averages"); cols_left.write(trending_avg)
    cols_right.write("Non-trending items averages"); cols_right.write(non_trending_avg)
